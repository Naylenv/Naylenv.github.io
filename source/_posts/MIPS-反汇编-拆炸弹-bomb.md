...
```
### 炸弹一
虽然<phase_1>的拆除思路在指导书中已大致给出，下面还是对该炸弹进行简要分析。结合对MIPS寄存器的了解（详细分析见结论分析与体会），可知$a0和$a1寄存器通常用来函数参数的传递。在<phase_1>中调用了函数<strings_not_equal>，并在调用前对$a0和$a1寄存器进行修改。分析汇编带代码：
